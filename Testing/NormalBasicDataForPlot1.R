...
set.seed(123456789)
Normal.Simulation.Output <- foreach (m = 1:Reps, .combine=rbind, .packages = c("data.table", "copula", "metafor"), 
  ...
  ID = length(tau.sq) * length(Subj) * length(theta) * sum(Studies)
...
Normal.Simulation.Output <- Normal.Simulation.Output[order(Unique_ID)]

TimeTakenSim <- proc.time() - StartTime

saveRDS(Normal.Simulation, file = "NSBasicDataBaseline")

##### Will need to add values in using code from PilotAnalysisNormal1.R - actually this will need to altered to account for different variables

#### Need to alter variables such that only useful data stored ?only baseline - doesn't need separation by number of studies or repeats necessarily

## Sample size distributions, ?empirical and root n 20-1000?