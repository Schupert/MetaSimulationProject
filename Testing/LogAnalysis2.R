...
# 
# LogOR.Sim.Results$Doi_CIlb <- LogOR.Sim.Results$FE_Estimate - qnorm(sig.level) * LogOR.Sim.Results$HC_DL_se
# LogOR.Sim.Results$Doi_CIub <- LogOR.Sim.Results$FE_Estimate + qnorm(sig.level) * LogOR.Sim.Results$HC_DL_se
# 
...
# Bias.values <- LogOR.Sim.Results[, .(Bias_FE = mean(FE_Estimate, na.rm = TRUE) - Rep_theta, Bias_REML = mean(REML_Estimate, na.rm = TRUE) - Rep_theta,
#                                       Bias_DL = mean(DL_Estimate, na.rm = TRUE) - Rep_theta, Bias_Moreno = mean(Moreno_Estimate, na.rm = TRUE) - Rep_theta),
#                                   by = .(Rep_theta, Rep_NumStudies, Rep_tau.sq, Rep_Subj, Rep_ev_freq)]
# 
# MSE.values <- LogOR.Sim.Results[, .(MSE_FE = mean((FE_Estimate - Rep_theta)^2, na.rm = TRUE), MSE_REML = mean((REML_Estimate - Rep_theta)^2, na.rm = TRUE),
#                                      MSE_DL = mean((DL_Estimate - Rep_theta)^2, na.rm = TRUE), "MSE Moreno" = mean((Moreno_Estimate- Rep_theta)^2, na.rm = TRUE) ),
#                                  by = .(Rep_theta, Rep_NumStudies, Rep_tau.sq, Rep_Subj, Rep_ev_freq)]
# 
# Coverage.values <- LogOR.Sim.Results[, .(Coverage_FE = mean(CI.betw(Rep_theta, FE_CIlb, FE_CIub), na.rm = TRUE), Coverage_REML = mean(CI.betw(Rep_theta, REML_CIlb, REML_CIub), na.rm = TRUE),
...
# by = .(Rep_theta, Rep_NumStudies, Rep_tau.sq, Rep_Subj, Rep_ev_freq)]
# 
# Outside.values <- LogOR.Sim.Results[, .(Outside_FE = mean(CI.updown(Rep_theta, FE_CIlb, FE_CIub), na.rm = TRUE), Outside_REML = mean(CI.updown(Rep_theta, REML_CIlb, REML_CIub), na.rm = TRUE),
...
# 
# 
# Intermediate1 <- merge(MSE.values, Bias.values, by = c("Rep_theta", "Rep_NumStudies", "Rep_tau.sq", "Rep_Subj", "Rep_ev_freq"))
# Intermediate2 <- merge(Intermediate1, Outside.values, by = c("Rep_theta", "Rep_NumStudies", "Rep_tau.sq", "Rep_Subj", "Rep_ev_freq"))
# Total.results <- merge(Intermediate2, Coverage.values, by = c("Rep_theta", "Rep_NumStudies", "Rep_tau.sq", "Rep_Subj", "Rep_ev_freq"))
...
write.csv(LogOR.Complete, "LogORResults.csv")
