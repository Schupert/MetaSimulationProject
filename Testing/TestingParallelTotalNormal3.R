...
#
#
#Need to do something about Subj being list, since match no longer works in assigning IDs
#
# ? apply(sapply(Subj, function(vec) {i %in% vec}), 1, which.max) for lowest index
...
r <- foreach (m = 1:Reps, .combine=rbind, .packages = c("data.table"), 
              .export = c("Studies", "Subj", "True.sd",
                          "theta", "tau.sq", "controlProp", "UMD")) %dorng% {
      ...
      for (l in tau.sq){
        ...
        for (n in Studies){
          ...
          for (o in 1:n){
            ...
            Normal.Simulation[Unique_ID == counter, `:=` (Rep_Number= m, Rep_Subj = i,
...
registerDoParallel(c1)
set.seed(234)
r <- foreach (m = 1:Reps, .combine=rbind, 
              .packages = c("data.table"), 
              .export = c("Studies", "Subj", "True.sd", "theta", "tau.sq", "controlProp", "UMD")) %dorng% {
...
r <- foreach (m = 1:Reps, .combine=rbind, 
              .packages = c("data.table"), 
              .export = c("Studies", 
                          "Subj", "True.sd", "theta", "tau.sq", "controlProp", "UMD", "UMD.mult.out")) %dorng% {
