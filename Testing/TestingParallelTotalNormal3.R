    ...
    for (k in theta){
      ...
      for (l in tau.sq){
        ...
        for (n in Studies){
          ...
          for (o in 1:n){
            new.this.loop <- as.integer((match(i, Subj)-1) * length(theta) * length(tau.sq) * sum(Studies) * Reps + 
      ...
      for (l in tau.sq){
        ...
        for (n in Studies){
          ...
          for (o in 1:n){
            ...
            counter <- as.integer((match(i, Subj)-1) * length(theta) * length(tau.sq) * sum(Studies) * Reps + 
                                    (match(k, theta)-1) * length(tau.sq) * sum(Studies) * Reps +
                                    (match(l, tau.sq)-1) * sum(Studies) * Reps +
                                    (sum(Studies[0:(match(n, Studies)-1)]) + o -1) * Reps +
                                    m
...
set.seed(456)
r <- foreach (m = 1:Reps, .combine=rbind, .packages = c("data.table"), .export = c("Studies", "Subj", "True.sd",
                                                                                   "theta", "tau.sq", "controlProp", "UMD", "UMD.mult.out")) %dorng% {
