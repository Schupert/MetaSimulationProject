...
Normal.Sim.Results$Rep_NumStudies = rep(intermediate, times = ID/(Reps*sum(Studies)))
Normal.Sim.Results$Rep_tau.sq = rep(rep(tau.sq, each = Reps * sum(Studies)), times = ID/(Reps*sum(Studies)*length(tau.sq)))
Normal.Sim.Results$Rep_theta = rep( rep(theta, each = Reps * sum(Studies) * length(tau.sq)), times = length(Subj))

### Create keyable vector for Subj
Subj2 <- c(20, 4.2)
Normal.Sim.Results$Rep_Subj = rep(Subj2, each = ID / length(Subj))

saveRDS(Normal.Sim.Results, file = "FunnelMethRDS")

### Checking values

sum(is.na(Normal.Sim.Results))
Normal.Sim.Results[is.na(Normal.Sim.Results$Study_estimate)]
