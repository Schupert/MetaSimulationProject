...
setwd("D:/Stats/AFP/Pilot/LogNoBias5")
...
for (d in EvFreq){
  ...
  for (c in Subj){
    ...
    for (e in Studies){
      ...
      An.Cond <- LogOR.Sim.Results[Rep_Subj == c & Rep_ev_freq == d & Rep_NumStudies == e]
      ...
      subDir <- file.path(paste("EventRate",d, sep=""))
      
      dir.create(file.path(mainDir, subDir), recursive = TRUE, showWarnings = FALSE)
      
      setwd(file.path(mainDir, subDir))
      
      ggsave(paste("OR", Type.of.bias, "BiasBoxplotTheta", "ER", d, "Size", c, "Studies", d, ".png", sep = ""), p11, dpi = 300, device = "png", width = 8.01, height = 5.67, units = "in")
      
    }
    
    for (a in theta){
      
      An.Cond <- LogOR.Sim.Results[Rep_Subj == c & Rep_ev_freq == d & Rep_theta == a]
      ...
      subDir <- file.path(paste("EventRate",d, sep=""), paste("Theta", round(a, digits = 2), sep =""))
      
      dir.create(file.path(mainDir, subDir), recursive = TRUE, showWarnings = FALSE)
      
      setwd(file.path(mainDir, subDir))
      
      ggsave(paste("OR", Type.of.bias, "BiasBoxplotStudies", "ER", d, "Size", c, "Theta", round(a, digits = 2), ".png", sep = ""), p12, dpi = 300, device = "png", width = 8.01, height = 5.67, units = "in")
      ...
      for (b in tau.sq){
        ...
        subDir <- file.path(paste("EventRate",d, sep=""), paste("Theta", round(a, digits = 2), sep =""), paste("Tau", b, sep =""))
        
        dir.create(file.path(mainDir, subDir), recursive = TRUE, showWarnings = FALSE)
        
        setwd(file.path(mainDir, subDir))
        
        ggsave(paste("OR", Type.of.bias, "BiasBoxplot", "Th", round(a, digits = 2), "Tau", b, "ER", d, "Size", c, ".png", sep = ""), p10, dpi = 300, device = "png", width = 8.01, height = 5.67, units = "in")
        ...
        An.Cond <- LogOR.Sim.Results[Rep_theta == a & Rep_tau.sq == b & Rep_Subj == c]
        
        
        Bias.values.total <- An.Cond[, .(FE = FE_Estimate - Rep_theta, REML = REML_Estimate - Rep_theta,
                                         DL = DL_Estimate - Rep_theta, Moreno = Moreno_Estimate - Rep_theta),
                                     by = .(Rep_ev_freq, Rep_NumStudies)]
        
        
        Bias.values.total2 <- melt(Bias.values.total, id = c("Rep_ev_freq", "Rep_NumStudies"))
        
        
        
        p9 <- ggplot(Bias.values.total2, aes(x = as.factor(Rep_NumStudies), y = value, fill = variable ) ) +
          #geom_boxplot(alpha=1, outlier.shape = NA, position=position_dodge(1)) +
          geom_boxplot(alpha=1, outlier.shape = NA, coef = 0) +
          ggtitle("Bias") + xlab("Number of studies") + ylab("Bias") + 
          theme_bw() +
          coord_cartesian(ylim = c(-0.75,0.75))+
          facet_grid(.~ Rep_ev_freq)
        
        ggsave(paste("OR", Type.of.bias, "BiasBoxplotEvent", "Th", round(a, digits = 2), "Tau", b, "Size", c, ".png", sep = ""), p9, dpi = 300, device = "png", width = 8.01, height = 5.67, units = "in")
...
setwd("D:/Stats/AFP/Pilot/LogMethods4")

system.time(LogOR.Sim.Results <- readRDS(file = "LSTotalMethRDS"))
...
setwd("D:/Stats/AFP/Pilot/LogBegg4")
...
setwd("D:/Stats/AFP/Pilot/LogOutcome4")

system.time(LogOR.Sim.Results <- readRDS(file = "LSTotalOutRDS"))
...
setwd("D:/Stats/AFP/Pilot/LogStep4")
