...
LogOR_mult_out2 <- function(StudySize, Theta, Heterogeneity, Control_Prop, mu, frac, num.times){
  StudyLogOR <- rnorm(1, Theta, sqrt(Heterogeneity))
  Group1Size <- as.integer(Control_Prop*StudySize)
  Group2Size <- Group1Size
  EventFreq <- log(mu / (1 - mu))
  ...
  Study.est <- log((TGO1/TGO2) / (CGO1/CGO2))
  Study.se <- sqrt(1/TGO1 + 1/TGO2 + 1/CGO1 + 1/CGO2)
  Study.p.val <- pnorm(Study.est/Study.se)
  top <- order(Study.p.val)[1]
  return(c(CGO1[top], CGO2[top], TGO1[top], TGO2[top], Group1Size))
}
        ...
        for(k in theta){
          ...
          for (n in Studies){
            ...
            for (o in 1:n){
              ...
              x <- LogOR_mult_out2(Study_patientnumber, k, l, controlProp, j, Sd.split, Tested.outcomes)
...
saveRDS(LogOR.Sim.Results, file = "LSTotalOutRDS")
