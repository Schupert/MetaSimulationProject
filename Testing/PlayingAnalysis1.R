...
r <- foreach (m = 1:Reps, 
              ...
              .export = c("Studies", "Subj", "controlProp", "theta", "tau.sq", "EvFreq", "Normal.Simulation")
) %dopar% {
  ...
  Normal.Simulation.Results <- data.table(
      ...
      for (k in theta){
        ...
        for (l in tau.sq){
          ...
          for (n in Studies){
            ...
            meta.analysis.fe <- rma.uni(yi = temp.data$Study_estimate, vi = temp.data$Rep_sd^2, method = "FE")
            
            meta.analysis.reml <- rma.uni(yi = temp.data$Study_estimate, vi = temp.data$Rep_sd^2, method = "REML")
