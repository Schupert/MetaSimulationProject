###########################
# This script is set up to simulate log odds ratios with bias introduced in an attempt to model the bias due to 
# poor methodological quality. This involves each study drawing a number of biases (up to 2), then introducing a fixed bias
# per methodological flaw. The specific values are based on empirical evidence from Savovic et al and BRANDO.
#
...
rng <- RNGseq(length(Subj)*length(theta), 3456)
...
saveRDS(LogOR.Simulation, file = "LSBMethV1")
#write.csv(LogOR.Simulation, file = "LSBMethV1.csv")
...
# 
# write.csv(df.LogOR.Simulation, file = "LSBMethV1.csv")
rm(r)
...
setkey(LogOR.Simulation, "Unique_ID")
...
r <- foreach (i = Subj, .combine=rbind, .packages = c("data.table", "metafor"), 
              .export = c("Studies", "Subj", "True.sd", "Reps",
                          "theta", "tau.sq", "EvFreq", "LogOR.Simulation", "anyNA", ".psort", "mod.hc", "findIDLogSim")
...
#write.csv(LogOR.Sim.Results, file = "LSBMethV1An.csv")
saveRDS(LogOR.Sim.Results, file = "LSBMethV1An")
